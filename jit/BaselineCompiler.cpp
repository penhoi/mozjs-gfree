...
//used for debug native code generated by baseline compiler only.
char *scriptflag;
...
BaselineCompiler::compile()
{
    ...
    //scriptflag = (char*)malloc(512);
    //sprintf(scriptflag,  "#Baseline code for script %s:%d (%p)\n",
    ...
    baselineScript->setMethod(code, hasCookie, cookie);
...
BaselineCompiler::emitPrologue()
{
    ...
    masmprintf("Enter method at prologure\n");
    masmprintfnext(scriptflag);
...
{
    JitSpew(JitSpew_Codegen, "%s", __FUNCTION__);
    masmprintf("exit method at epilogure\n");
    masmprintfnext(scriptflag);
...
BaselineCompiler::emitBody()
{
    ...
    while (true) {
        ...
        if (op == JSOP_LOOPENTRY) {
            skipXorBP_ = new Label;
            masm.toggledJump(skipXorBP_);
        }
...
{
    //Label skipXorBP;
    //masm.toggledJump(&skipXorBP);
    //masmprintf("Just before loopentry\n");
    //size_t nentries = pcMappingEntries_.length();
    //pcMappingEntries_[nentries - 1].nativeOffset = masm.currentOffset();
    //masmprintfnext("pass through or enter at loopentry\n");
    masm.xorSPviaBP();
    masm.bind(skipXorBP_);
    if (skipXorBP_ != nullptr) {
        delete skipXorBP_;
        skipXorBP_ = nullptr;
    }
    frame.syncStack(0);
